...
export default defineComponent({
  setup(){
    ...
    const fetchProductList = async () => {
      ...
      abbreviation.value = data[0].customer.customer_abbreviation
      order.value = data[0].sale.customer_order
      address.value = data[0].sale.goods_address
    };
    // 统计合计
    ...
    const setPrintTableTr = () => {
      const Dom = tableData.value.map(e => {
        const tr = <>
          <tr class="table-row">
            <td>{ e.product.product_code }</td>
            <td>{ e.product.product_name }</td>
            <td>{ e.product.model } & {e.product.specification}</td>
            <td>{ e.product.other_features }</td>
            <td>{ e.sale.unit }</td>
            <td>{ e.sale.order_number }</td>
            <td>{ modelSwitch.value ? "***" : e.product.unit_price }</td>
            <td>{ modelSwitch.value ? "***" : Number(e.sale.order_number) * Number(e.product.unit_price) }</td>
          </tr>
        </>
        return tr
      })
      const Dom2 = <>
        <tr class="table-row">
          ...
          <td colspan="6" style={{ textAlign: 'left' }}>{() => {
            const totalPrice = tableData.value.reduce((sum, item) => {
              const order_number = Number(item.sale.order_number)
              const unit_price = Number(item.product.unit_price)
              return sum + order_number * unit_price
            }, 0);
            return '人民币大写：' + (modelSwitch.value ? "***" : numberToChinese(totalPrice))
          }}</td>
          <td>{{
            default: () => {
              const totalPrice = tableData.value.reduce((sum, item) => {
              ...
              return modelSwitch.value ? "***" : totalPrice
            }
          }}</td>
        </tr>
      </>
              ...
              <>
                ...
                <div class="printTable" id='totalTable2'>
                  <div id="printTable">
                      ...
                      <thead>
                        <tr>
                          <th colspan="8" class="title-cell">
                            <div class="popTitle" style={{ textAlign: 'center', fontSize: '36px' }}>出货单</div>
                          </th>
                        </tr>
                        <tr>
                          <th colspan="8" class="header-cell">
                            <div class="flex row-between print-header">
                              <div>客户名称:{ abbreviation.value }</div>
                              <div>客户订单号:{ order.value }</div>
                              <div>交货地点:{ address.value }</div>
                            </div>
                          </th>
                        </tr>
                    ...
                    <div id="extraPrintContent" class="flex" style="justify-content: space-between; padding-top: 6px;">
                      <div>客户签收：</div>
                      <div>审查：</div>
