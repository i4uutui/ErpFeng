  ...
  setup(){
    ...
    return() => (
      <>
                ...
                {{
                  left: () => (
                    <ElFormItem v-permission={ 'PartCode:add' }>
                      <ElButton type="primary" onClick={ handleAdd }>新增部件编码</ElButton>
                    </ElFormItem>
                  ),
                  center: () => (
                    <>
              ...
              <>
                <ElTable data={ tableData.value } border stripe height={ `calc(100vh - ${formHeight.value + 224}px)` } style={{ width: "100%" }}>
                  ...
                  <ElTableColumn label="操作" width="140" fixed="right">
                    {(scope) => (
                      <>
                        <ElButton size="small" type="warning" v-permission={ 'PartCode:edit' } onClick={ () => handleUplate(scope.row) }>修改</ElButton>
        ...
        <ElDialog v-model={ dialogVisible.value } title={ edit.value ? '修改部件编码' : '新增部件编码' } width='785' center draggable onClose={ () => handleClose() }>
          {{
            default: () => (
