  ...
  setup(){
    ...
    return() => (
      <>
              ...
              <>
                <ElTable data={ tableData.value } border stripe rowStyle={ handleRowStyle } height={ `calc(100vh - ${formHeight.value + 224}px)` } style={{ width: "100%" }}>
                  ...
                  <ElTableColumn label="操作" width='140' fixed="right">
                    {(scope) => (
                      <>
                        <ElButton size="small" type="warning" v-permission={ 'EquipmentCode:edit' } onClick={ () => handleUplate(scope.row) }>修改</ElButton>
        ...
        <ElDialog v-model={ dialogVisible.value } title={ edit.value ? '修改设备信息' : '新增设备信息' } width='815' center draggable onClose={ () => handleClose() }>
          {{
            default: () => (
              <ElForm class="ml20" model={ form.value } ref={ formRef } inline={ true } rules={ rules } label-width="115px">
                ...
                <ElFormItem label="制程组" prop="cycle_id">
                  <ElSelect v-model={ form.value.cycle_id } multiple={false} filterable remote remote-show-suffix placeholder="请选择制程组">
